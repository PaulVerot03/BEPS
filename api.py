...
# Setup MongoDB Connection Dependency
# In production, we'd manage the client lifecycle on app startup/shutdown
async def get_collection():
    load_dotenv()
    MONGO_URI = os.getenv("MONGO_URI")
    # Using tls=True for cloud MongoDB connection as in original code
    client = AsyncMongoClient(MONGO_URI, tls=True)
    # The database is 'anais' and collection 'sequence' (based on user output)
    db = client["anais"]
    return db["sequence"]
